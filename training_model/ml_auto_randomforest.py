        ...
        for iteration in range(2,40):
            for ii in range(2,30):
                for num in range(2,5):
                    clf = RandomForestClassifier(max_depth = ii , random_state = num,n_jobs = -1,n_estimators = iteration,bootstrap = True)
                    ...
                    if ( matrix[2,1:].sum() > 3 and matrix[0,2] < 98 ) or (matrix[1,1:].sum() > 4 and matrix[0,2] < 98) or (matrix[0,2] < 3 and matrix[3,1:].sum() == 1):
                        ...
                        config_list.append('randomstate : '+str(num)+' estimators : '+str(iteration)+' depth : '+str(ii)+' precision : '+str(precis)+' recall : '+str(recall)+' class3 : '+str(matrix[2][2]))
                    if ( f1score > 0.38 ):
                        config_list.append(np.array_str(matrix))
                        config_list.append('randomstate : '+str(num)+' estimators : '+str(iteration)+' depth : '+str(ii)+' precision : '+str(precis)+' recall : '+str(recall)+' class3 : '+str(matrix[2][2]))
                        
        with open('randomforestconfig' + " %s" % undersample_percent +".txt",'w') as wfile:
